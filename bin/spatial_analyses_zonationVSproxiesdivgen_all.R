...
# Path to rasters
myrasters<-c("../data/spatial/Zonation_output/00_MDP_PDG_caz.rank.compressed.tif", # Zonation output proxis div gen + species (n=221)
             "../data/spatial/Zonation_output/03_caz.rank.compressed.tif", # Zonation output Zonas Vida + species
             "../data/spatial/Zonation_output/00_MDP_caz.rank.compressed.tif", # Zonation output with species only (n=119)
             "../data/spatial/Zonation_output/01.rank.compressed.tif", # Zonation output species vs proxies (n=12240)
             "../data/spatial/Zonation_output/00_MDP_ADMU.ADMU.redistributed.rank.compressed.tif", # Zonation output with species, incorporating proxies as "administrative units" (n=119)
...
CWR<-stack(myrasters)
names(CWR)<-c("ZonationProxiDG", "ZonationZV", "ZonationSPP", "ZonationProxiDGvsSPP", "ZonationSPP_ADMU", "ProxiesDivGen")

#### Data description #####

ZT=0.10 #Zonation threshold
...
plot(CWR$ZonationSPP>=(1-ZT), legend=FALSE, main= "Zonation output Spp models")
plot(CWR$ZonationZV>=(1-ZT), legend=FALSE, main= "Zonation output Life Zones")
plot(CWR$ZonationProxiDG>=(1-ZT), legend=FALSE, main= "Zonation output Proxies Genetic Diversity")
plot(CWR$ZonationProxiDGvsSPP>=(1-ZT), legend=FALSE, main= "Zonation output Proxies Genetic Diversity vs spp")
plot(CWR$ZonationSPP_ADMU>=(1-ZT), legend=FALSE, main= "Zonation output Spp models, Proxies as ADMU")
...
spp<-list.files("../data/spatial/modelos_darwin_all/")
spp
...
### calcs
## subet
x<-dplyr::filter(CWR_overlaps, ENM_sp==1, ZonationSPP>=(1-ZT)) %>% arrange(., ProxiesDivGen)
x$ProxiesDivGen<-as.character(x$ProxiesDivGen)

# Save to common data frame
sol_ZSPP<-as.data.frame(table(x$ProxiesDivGen))
names(sol_ZSPP)<-c("ProxiDivGen", "ZonationSPP")
...
### calcs
## subet
x<-dplyr::filter(CWR_overlaps, ENM_sp==1, ZonationZV>=(1-ZT)) %>% arrange(., ProxiesDivGen)
x$ProxiesDivGen<-as.character(x$ProxiesDivGen)


# Save to common data frame
sol_ZZV<-as.data.frame(table(x$ProxiesDivGen))
names(sol_ZZV)<-c("ProxiDivGen", "ZonationZV")
...
### calcs
## subet
x<-dplyr::filter(CWR_overlaps, ENM_sp==1, ZonationProxiDG>=(1-ZT)) %>% arrange(., ProxiesDivGen)
x$ProxiesDivGen<-as.character(x$ProxiesDivGen)

# Save to common data frame
sol_ZProxiDG<-as.data.frame(table(x$ProxiesDivGen))
names(sol_ZProxiDG)<-c("ProxiDivGen", "ZonationProxiDG")
...
##### 5) CWRsp + Zonation Zonas Proxies Div Gen vs SPP

### calcs
## subet
x<-dplyr::filter(CWR_overlaps, ENM_sp==1, ZonationProxiDGvsSPP>=(1-ZT)) %>% arrange(., ProxiesDivGen)
x$ProxiesDivGen<-as.character(x$ProxiesDivGen)


# Save to common data frame
sol_ZProxiDGvsSPP<-as.data.frame(table(x$ProxiesDivGen))
names(sol_ZProxiDGvsSPP)<-c("ProxiDivGen", "ZonationProxiDGvsSPP")
...
##### 6) CWRsp + Zonation spp models + Proxies Div Gen as administrative units 

### calcs
## subet
x<-dplyr::filter(CWR_overlaps, ENM_sp==1, ZonationSPP_ADMU>=(1-ZT)) %>% arrange(., ProxiesDivGen)
x$ProxiesDivGen<-as.character(x$ProxiesDivGen)


# Save to common data frame
sol_ZSPP_ADMU<-as.data.frame(table(x$ProxiesDivGen))
names(sol_ZSPP_ADMU)<-c("ProxiDivGen", "ZonationSPP_ADMU")
...
## Prepare data
# Join all solutions
sol_all<-left_join(sol_base, sol_ZSPP) %>% left_join(., sol_ZZV) %>% left_join(., sol_ZProxiDG) %>% left_join(., sol_ZProxiDGvsSPP) %>% left_join(., sol_ZSPP_ADMU) %>%
...
sol_prop<-mutate(sol_all,
                 ZonationSPP=ZonationSPP/ENM_sp, 
                 ZonationZV=ZonationZV/ENM_sp,
                 ZonationProxiDG=ZonationProxiDG/ENM_sp,
                 ZonationProxiDGvsSPP=ZonationProxiDGvsSPP/ENM_sp,
                 ZonationSPP_ADMU=ZonationSPP_ADMU/ENM_sp)
head(sol_prop)
...
sol_dist<-data.frame(Solution=c("ENM_sp", "ZonationSPP", "ZonationZV", "ZonationProxiDG" ,"ZonationProxiDGvsSPP" ,"ZonationSPP_ADMU"),
                     Dist.to.Optimun=c(as.vector(dist(rbind(sol_all$ENM_sp, sol_all$ENM_sp))),
                                       as.vector(dist(rbind(sol_all$ENM_sp, sol_all$ZonationSPP))),
                                       as.vector(dist(rbind(sol_all$ENM_sp,sol_all$ ZonationZV))),
                                       as.vector(dist(rbind(sol_all$ENM_sp, sol_all$ZonationProxiDG))),
                                       as.vector(dist(rbind(sol_all$ENM_sp, sol_all$ZonationProxiDGvsSPP))),
                                       as.vector(dist(rbind(sol_all$ENM_sp, sol_all$ZonationSPP_ADMU)))))
...
filter(sols_summary_spp, Solution!="ENM_sp") %>%
  ...
  scale_x_continuous(name="Proportion of distribution of the sp conserved") +
  scale_y_continuous(name="Mean proportion of the area of each proxy conserved by sp") + geom_smooth(method=loess, aes(fill=Solution))
